...
class GameEngine {
    ...
    static async initialize(gameSessionId) {
        const session = await prisma_1.prisma.gameSession.findUnique({
    ...
    async handleDeploy(playerId, data) {
        ...
        if (data.cardType === "Character" && "power" in card) {
            player.deployedCharacters.push(card);
        }
        else if (data.cardType === "Item" && "effect" in card) {
            player.deployedItems.push(card);
        }
    ...
    getDefeatedCharacterCount(player) {
    ...
    async saveState(gameSessionId) {
        ...
        for (const [userId, playerState] of Object.entries(this.state.players)) {
            await prisma_1.prisma.gameSessionPlayer.updateMany({
                where: {
                    gameSessionId,
                    userId,
                },
